...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PATENTMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PATENTMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PATENTMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("patent-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("PATENTMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running PATENTMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadPATENTMiner() exception");
    } catch (...) {
        LogPrintf("ThreadPATENTMiner() exception");
    }

    LogPrintf("ThreadPATENTMiner exiting\n");
}
